...
The pre-publication fetch confirmed `origin/v2` still at the original SHA and
both published archive refs intact. Upstream advanced to
`d1d1c6f890cee88677d81028da6ed5fcc55ee4ce`. This publication retains the reviewed
and tested `a55dc8c84a` base rather than adding untested upstream changes.
Integration of that newer upstream tip is deferred.
...
3. The pre-publication fetch verified remote `v2` at the original SHA.
   Incorporate the newer upstream tip in a subsequent reviewed restack and
   repeat affected validation; it is not part of this publication.
4. Publish only with the exact original-remote lease:

   ```sh
   git push origin HEAD:refs/heads/v2 \
     --force-with-lease=refs/heads/v2:e524b3ac177f204a13a68b04d66c1669e4131fb4
   ```

   A lease rejection requires inspecting and preserving new remote work.
   Do not simply substitute a fresh lease.
5. In Starter, use `opencode:bump <published-sha>` and `opencode:sync`, run its
   frozen install, typechecks, tests, and host contracts, then review and
   fast-forward its local `main` before pushing `origin/main`.
6. Inspect primary checkout branches, dirty state, and worktrees before
