...
The original publication was based on `a55dc8c84a`. Two subsequent fork
restacks replay the complete retained range over newer upstream V2 tips. The
latest candidate is based on `d39290fbb9a57c442fd8a51f2e147b50a81178d8` and is
not published until the rebase validation completes.
...
3. Publish the SSH upstream rebase only with the exact published-fork lease:

   ```sh
   git push origin HEAD:refs/heads/v2 \
     --force-with-lease=refs/heads/v2:ca2bb01fb2131ea740d5be427bc010485b57ac04
   ```
