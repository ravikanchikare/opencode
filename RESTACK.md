...
The fork stack has been rebuilt on `upstream-rebase`. The user authorized
...
The original publication was based on `a55dc8c84a`. Subsequent fork restacks
replay the complete retained range over newer upstream V2 tips. The latest
candidate is based on `c0cb1c7a919fedb6bc2bed92bd072a3ebfbf9d8a` and is not
...
## Latest upstream V2 rebase

| Ref | SHA |
| --- | --- |
| Fork `v2` before this rebase | `f8692371ccc9e63cc3e130ced48e550a098b2dae` |
| Current fetched `upstream/v2` | `6ee2ed7510c971415a312273c9bf9e2364c6d948` |
| Rebase merge base | `8a5709324f847e5cb9fd1bb8b088f5e8330630a6` |

Before rewriting, the following annotated tag and branch were pushed, both
targeting the published fork tip:

- `refs/heads/archive-v2-20260909-065430`
- `refs/tags/archive-v2-20260909-065430`

All 16 retained fork commits replayed in their existing order. The only
conflict was in the permission dock: retain the fork's `request.save` guard
for "Always allow" and upstream's contrast "Allow once" action styling.
The final upstream advance adds account switching and removes the session
content-mutation API; it replayed without fork conflicts.

## SSH upstream V2 rebase

| Ref | SHA |
| --- | --- |
| Fork `v2` before this rebase | `ca2bb01fb2131ea740d5be427bc010485b57ac04` |
| Current fetched `upstream/v2` | `d39290fbb9a57c442fd8a51f2e147b50a81178d8` |
| Rebase merge base | `6ee2ed7510c971415a312273c9bf9e2364c6d948` |

The annotated tag and branch `archive-v2-20260909-070537` were pushed before
rewriting and both target the previous published fork tip.

The upstream SSH-connection feature conflicted only with the fork's home
composition imports. The resolution retains upstream's SSH authentication and
project styling imports alongside the fork's support-link and composition
imports. All 16 retained commits remain linear.

## CodeMode upstream V2 rebase

| Ref | SHA |
| --- | --- |
| Fork `v2` before this rebase | `1f2df4bf2e0aef531b49930de8a52d76f690c2cf` |
| Current fetched `upstream/v2` | `08e28fb915d7a4d64add09150fc4ee7d63a796ef` |
| Rebase merge base | `d39290fbb9a57c442fd8a51f2e147b50a81178d8` |

The annotated tag and branch `archive-v2-20260909-083136` were pushed before
rewriting and both target the previous published fork tip.

Upstream's CodeMode data-boundary refactor conflicted with the retained
`tools.search` alias. The resolution keeps upstream's `toData` conversion and
prepared tool handling, while applying the alias only when no registered
one-segment `search` tool shadows it. The fix remains in its owning CodeMode
commit; no behavioral patches were dropped.

## Composer upstream V2 rebase

| Ref | SHA |
| --- | --- |
| Fork `v2` before this rebase | `815e6af519d0f587c4cf048ade141dd65c182bc0` |
| Current fetched `upstream/v2` | `c0cb1c7a919fedb6bc2bed92bd072a3ebfbf9d8a` |
| Rebase merge base | `08e28fb915d7a4d64add09150fc4ee7d63a796ef` |

The annotated tag and branch `archive-v2-20260909-211358` were pushed before
rewriting and both target the previous published fork tip.

Upstream's model-picker keyboard-navigation test conflicted with the fork's
catalog-default behavior. The retained test asserts that a new project begins
with the configured catalog default, then uses upstream's focus and ArrowUp
interaction to change it. All 16 retained commits remain linear.

Upstream's compaction schema preserves `tokens` on the stored completed event,
but its current projected TypeScript type does not surface that optional field.
The focused Core test therefore asserts the complete persisted record rather
than accessing an unavailable property. This is one intentional test-only fork
commit to keep the rebased stack typecheckable.
...
### Latest upstream V2 rebase validation

| Check | Result |
| --- | --- |
| Root `bun install --frozen-lockfile` | Passed with no lockfile change |
| Full typecheck | 35 packages passed |
| Core permission and worktree suites | 139 passed |
| App composition and plugin-option suites | 19 passed |
| CodeMode tool-runtime suite | 53 passed |
| Desktop packaging and updater-provider suite | 32 passed |
| Update service typecheck | Passed; no package test files exist |
| `git range-diff` and `git diff --check upstream/v2..HEAD` | Passed |

### SSH upstream V2 rebase validation
...
### CodeMode upstream V2 rebase validation

| Check | Result |
| --- | --- |
| CodeMode typecheck | Passed |
| CodeMode tool-runtime suite | 1,143 passed |
| `git range-diff` and `git diff --check upstream/v2..HEAD` | Passed |

### Composer upstream V2 rebase validation

| Check | Result |
| --- | --- |
| Root `bun install --frozen-lockfile` | Passed with no lockfile change |
| App typecheck | Passed |
| Core typecheck and native compaction suite | Passed; 5 tests |
| `git range-diff` and `git diff --check upstream/v2..HEAD` | Passed |
...
3. Publish the CodeMode upstream rebase only with the exact published-fork
   lease:

   ```sh
   git push origin HEAD:refs/heads/v2 \
     --force-with-lease=refs/heads/v2:1f2df4bf2e0aef531b49930de8a52d76f690c2cf
   ```
