...
test("list and detail page share the availability controls and mutation owner", () => {
  ...
  expect(panel).toContain("onEnabledChange={(enabled) => void inventory.set(skill(), enabled)}")
  expect(panel).toContain("onChange={(enabled) => void inventory.set(item, enabled)}")
