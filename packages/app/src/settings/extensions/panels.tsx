...
import { currentSkill, detailOf, payloadFor, scopeOf, type SkillRow } from "./skill-availability"
...
import { ExtensionDestination, ExtensionList, ExtensionRow, ExtensionSections } from "./shell"
      ...
      fallback={
        ...
        >
          ...
          <Show when={(!plugins.loading() && !plugins.error()) || plugins.rows().length > 0}>
            ...
            >
              <ExtensionSections sections={sections()} empty={language.t("settings.plugins.none")}>
                {row}
              </ExtensionSections>
            </Show>
          </Show>
...
export const SkillsPanel: Component<ExtensionPanelProps> = (props) => {
  ...
  const sections = createMemo(() => skillInventoryGroups(inventory.rows()))
  const grouped = createMemo(() => sections().some((section) => section.group !== undefined))
  const row = (item: SkillRow) => (
    <ExtensionRow
      icon="post-skill"
      name={item.name}
    ...
    <Show
      ...
      fallback={
        ...
        >
          ...
          >
            <ExtensionSections sections={sections()} empty="No skills are available">
              {row}
            </ExtensionSections>
          </Show>
        </ExtensionDestination>
