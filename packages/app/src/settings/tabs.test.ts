...
describe("composeProjectSettingsNavItems", () => {
  const project = [item("general"), item("workspaces"), item("extensions")]
  const composition = {
    add: [
      { ...tab("skills"), panel: "skills" as const, content: undefined },
      { ...tab("mcp"), panel: "mcp" as const, content: undefined },
      { ...tab("plugins"), panel: "plugins" as const, content: undefined },
    ],
  }

  test("keeps the stock Extensions destination without a composition", () => {
    expect(composeProjectSettingsNavItems(project)).toEqual(project)
  })

  test("replaces Extensions with the three composed native destinations", () => {
    expect(composeProjectSettingsNavItems(project, composition).map((entry) => entry.value)).toEqual([
      "general",
      "workspaces",
      "skills",
      "mcp",
  ...
  test("keeps Extensions unless all three native panels are promoted", () => {
    expect(composeProjectSettingsNavItems(project, { add: composition.add.slice(1) }).map((entry) => entry.value)).toEqual(
      ["general", "workspaces", "extensions"],
    )
  })

  test("maps old Extensions deep links to composed destinations", () => {
    expect(projectExtensionDestination("mcps", composition)).toBe("mcp")
    expect(projectExtensionDestination("plugins", composition)).toBe("plugins")
    expect(projectExtensionDestination("skills", composition)).toBe("skills")
    expect(projectExtensionDestination("lsps", composition)).toBe("extensions")
    expect(projectExtensionDestination(undefined, composition)).toBe("skills")
    expect(projectExtensionDestination("mcps")).toBe("extensions")
  })
})

describe("composeServerSettingsNavItems", () => {
  const server = ["general", "projects", "workspaces", "providers", "models", "extensions"].map(item)
  const panels = [
    { ...tab("skills"), panel: "skills" as const, content: undefined },
    { ...tab("mcp"), panel: "mcp" as const, content: undefined },
    { ...tab("plugins"), panel: "plugins" as const, content: undefined },
  ...
  test("keeps the stock server destinations without a composition", () => {
    expect(composeServerSettingsNavItems(server)).toEqual(server)
  })

  test("promotes composed panels in place of Extensions and appends the rest", () => {
    expect(
      composeServerSettingsNavItems(server, { add: [...panels, tab("code-references")] }).map((entry) => entry.value),
    ).toEqual([
      "general",
      "projects",
      "workspaces",
  ...
  test("keeps Extensions and appends partial panel sets", () => {
    expect(composeServerSettingsNavItems(server, { add: panels.slice(1) }).map((entry) => entry.value)).toEqual([
      "general",
      "projects",
      "workspaces",
      "providers",
