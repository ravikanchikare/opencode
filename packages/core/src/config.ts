  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const discover = Effect.fn("Config.discover")(function* () {
        ...
        // Stock OpenCode also reads ~/.claude and ~/.agents. A branded
        // distribution stays inside its own identity directory plus project
        // `.opencode` so it cannot pick up another product's config.
        const claude = stock
          ? [
              ...new Set([
