...
test("reports a location resource as pending until its sync settles", async () => {
  ...
  try {
    ...
    const syncing = setup.data.location.provider.sync(location)
    expect(setup.data.location.provider.pending(location)).toBe(true)
    ...
    expect(setup.data.location.provider.pending(location)).toBe(false)
    expect(setup.data.location.provider.list(location)).toEqual([])
  } finally {
    setup.dispose()
  }
})
