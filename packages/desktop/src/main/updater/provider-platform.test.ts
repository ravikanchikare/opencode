...
describe("packaged updater provider", () => {
  ...
  test("forwards config verbatim beside the host's own options", async () => {
    ...
    expect(await Effect.runPromise(platform.checkForUpdate)).toEqual({ mode: "restart", version: "2.0.0" })
    await Effect.runPromise(platform.stageUpdate({ differential: true }))
    platform.dispose()
