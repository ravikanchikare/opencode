...
import { configureManagedPlugins, managedPluginConfig, managedPluginPaths } from "./managed-plugins"

const scratch = () => mkdtempSync(join(tmpdir(), "managed-plugins-"))

function source(
  plugins: { readonly id: string; readonly file: string; readonly contents: string }[],
  skillSourceAdmission?: { readonly externalHarnesses: boolean },
) {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      version: 1,
      ...(skillSourceAdmission ? { skillSourceAdmission } : {}),
...
describe("configureManagedPlugins", () => {
  test("publishes only verified app-resource paths to the server environment", () => {
    ...
    expect(JSON.parse(env.OPENCODE_MANAGED_SKILL_SOURCE_ADMISSION ?? "{}")).toEqual({ externalHarnesses: true })
  })

  test("publishes only manifest-carried skill source admission policy", () => {
    const directory = source([{ id: "acme.a", file: "acme.a.js", contents: "a" }], {
      externalHarnesses: false,
    })
    const env = { OPENCODE_MANAGED_SKILL_SOURCE_ADMISSION: '{"excludeCompatibility":[]}' }

    configureManagedPlugins({ packaged: false, resourcesPath: scratch(), source: directory }, env)

    expect(
      managedPluginConfig({ packaged: false, resourcesPath: scratch(), source: directory }).skillSourceAdmission,
    ).toEqual({
      externalHarnesses: false,
    })
    expect(JSON.parse(env.OPENCODE_MANAGED_SKILL_SOURCE_ADMISSION ?? "{}")).toEqual({
      externalHarnesses: false,
    })
  })
  ...
  test("replaces an ambient managed source when the app has no managed resources", () => {
    const env = {
      OPENCODE_MANAGED_PLUGINS: '["/tmp/unverified.js"]',
      OPENCODE_MANAGED_SKILL_SOURCE_ADMISSION: '{"externalHarnesses":false}',
    }

    expect(configureManagedPlugins({ packaged: false, resourcesPath: scratch() }, env)).toEqual([])
    expect(env.OPENCODE_MANAGED_PLUGINS).toBe("[]")
    expect(env.OPENCODE_MANAGED_SKILL_SOURCE_ADMISSION).toBe('{"externalHarnesses":true}')
  })
})
