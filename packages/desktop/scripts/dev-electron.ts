...
export function devElectronIconPath(env: NodeJS.ProcessEnv = process.env) {
  const directory = env.OPENCODE_DESKTOP_ICON_DIR?.trim()
  return directory ? join(directory, "icon.icns") : join(import.meta.dirname, "../icons/dev/icon.icns")
}
...
export async function prepareDevElectron() {
  ...
  const icon = Bun.file(devElectronIconPath())
